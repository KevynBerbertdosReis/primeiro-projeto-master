...
describe('page/Register', () => {
    it('Testando renderização dos itens do formulário', () => {
        ...
        cy.get('label').should('have.length', 5).each((item, index) => {
            cy.wrap(item).should('have.text', label[index]);
        })

        cy.get('input[type="text"]').should('have.length', 2).each((item, index) => {
            cy.wrap(item).should('have.attr', 'placeholder').and('include', placeholder[index])
        })
        ...
        cy.get('a').should('have.length', 2).and('have.text', "Clique aqui")
    })
})